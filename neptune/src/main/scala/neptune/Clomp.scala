...
object ImageHashing {
    ...
    def averageHash(image: Mat): String = image.toString() // Placeholder, substitua com seu código de hash
...
object VideoProcessor {
  ...
  def listVideosInDirectory(directoryPath: String): List[String] = {
    ...
    val videoFiles = new java.io.File(directoryPath).listFiles()
      .filter(file => videoExtensions.exists(ext => file.getName.toLowerCase.endsWith(ext)))
    ...
    def naturalSort(l: List[String]): List[String] = {
        ...
        val convert: String => Any = text => if (text.matches("\\d+")) text.toInt else text.toLowerCase

        // Função que divide a string em partes (números e não-números) e faz a conversão
        val alphanumKey: String => List[Any] = key => "\\D+|\\d+".r.findAllIn(key).map(convert).toList
  ...
  def videoHashes(path: String): List[Map[String, String]] = {
    ...
    val frameCount = vidCap.get(CAP_PROP_FRAME_COUNT).toInt
    val fps = vidCap.get(CAP_PROP_FPS)
    val duration = (frameCount / fps * 1000).toInt
...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val directoryPath = "/mnt/c/users/breno/downloads/ghqe"
